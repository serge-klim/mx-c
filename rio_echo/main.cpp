...
void setup_common_options(boost::program_options::options_description& description)
{
   ...
   description.add_options()
       ...
       ("if-out,i", boost::program_options::value<std::string/*boost::asio::ip::address*/>()/*->required()*/, "interface to multicast on")
       ("source,s", boost::program_options::value<net::endpoint>()->required(), "\taddress to listen to ip:port")
       ("if-in", boost::program_options::value<std::string>(), "\tinterface to listen to on")
       ("ttl", boost::program_options::value<unsigned short>(), "\tmulticast ttl")
       ("numa-node", boost::program_options::value<unsigned long>(), "\tnuma node")
       ("queue-size,q", boost::program_options::value<std::size_t>()->default_value(20), "\tqueue size")
       //("number-of-packets,n", boost::program_options::value<std::size_t>()->default_value(100), "\tnumber of packets to send")
       //("max-retries,r", boost::program_options::value<std::size_t>()->default_value(11), "\tnumber of packets to send")
       ("payload-size", boost::program_options::value<std::size_t>()->default_value(1536), "\tbuffer size")
       ("loopback", boost::program_options::value<bool>()->default_value(false), "\tenable loopback")
       ("dont-warmup", boost::program_options::value<bool>()->default_value(false), "\tdon't send warm up messages on start")
   // clang-format on
     ;
}
